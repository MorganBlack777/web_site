...
async function getAQI(lat, lon) {
  ...
  try {
    ...
    if (aqi <= 50) {
      aqiDescription = 'Хороший';
      aqiColor = '#00e400';
    } else if (aqi <= 100) {
      aqiDescription = 'Умеренный';
      aqiColor = '#ffff00';
    } else if (aqi <= 150) {
      aqiDescription = 'Вредный для определенныз групп людей';
      aqiColor = '#ff7e00';
    } else if (aqi <= 200) {
      aqiDescription = 'Вредный';
      aqiColor = '#ff0000';
    } else if (aqi <= 300) {
      aqiDescription = 'Очень вредный';
      aqiColor = '#8f3f97';
    } else {
      aqiDescription = 'Очень опасный';
      aqiColor = '#7e0023';
    }

    aqiInfo.innerHTML = `
      <p>Индекс качества воздуха: <span style="color: ${aqiColor}; font-weight: bold;">${aqi} - ${aqiDescription}</span></p>
    `;
